...
`bee-mcp` — a stdio MCP server exposing 8 LLM tools (`bee_chat`, plus a code
specialist, a security specialist, a research specialist, documents RAG, and
personal memory). It is a thin client:
tool calls are forwarded to the hosted Bee gateway (`https://bee.heossi.com/bee`),
...
1. The server "bee" connects and lists **8 tools** (bee_chat, bee_code,
   bee_security, bee_research, bee_documents_search, bee_documents_add,
   bee_memory_search, bee_memory_add) and **4 resources**
